...
SettingsGui_GetCapsLockText() {
    ...
    text .= "Исправляет регистр выделенного текста.`r`n"
    text .= "`r`n"
    text .= "Функция работает через выделение:`r`n"
    text .= "выделенный текст → Clipboard → обработка → вставка обратно.`r`n"
    text .= "`r`n"
    ...
    text .= "GitHUB → GitHub, если GitHub есть в exclude.txt`r`n"
    ...
    text .= "Например, PowerShell может быть восстановлен именно как PowerShell.`r`n"
    text .= "`r`n"
    text .= "Дополнительные режимы регистра будут отдельным этапом.`r`n"
